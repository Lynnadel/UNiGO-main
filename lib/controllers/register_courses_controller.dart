...
class RegisterCoursesController extends ChangeNotifier {
  ...
  Future<void> loadInitialData() async {
    ...
    try {
      ...
      if (facultyId.isEmpty) throw Exception('User facultyId is missing.');

      final enrolledRaw = (userData['enrolledCourses'] ?? []) as List<dynamic>;
      final currentCourseCodes = enrolledRaw
          .map((e) => e.toString().trim())
          .toSet();

      final upcomingRaw = (userData['upcomingCourses'] ?? []) as List<dynamic>;
      final upcomingCodes = upcomingRaw.map((e) => e.toString().trim()).toSet();
      ...
      for (final doc in coursesSnap.docs) {
        ...
        if (rawSections is List) {
          sections = rawSections
              .whereType<Map>()
              .map((m) => CourseSection.fromMap(Map<String, dynamic>.from(m)))
              .toList();
        }
  ...
  Future<String?> addSubject(Subject subject, {CourseSection? section}) async {
    ...
    if (code == null) return 'Invalid course (missing code).';
    ...
    try {
      ...
      registeredSubjects.add(subject.copyWith(selectedSection: section));
      notifyListeners();
      return null;
    } catch (e) {
  ...
  Future<String?> replaceSubject(Subject oldSubject, Subject newSubject) async {
    ...
    if (oldCode == null || newCode == null || oldCode == newCode) return null;
    ...
    try {
      ...
      await _db.runTransaction((tx) async {
        ...
        if (!codes.contains(newCode)) codes.add(newCode);
