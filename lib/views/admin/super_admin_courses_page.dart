...
class _SuperAdminCoursesPageState extends State<SuperAdminCoursesPage> {
  ...
  Future<void> _toggleAvailableNextSemester(
    ...
    try {
      await _coursesCol.doc(doc.id).set({
  ...
  Future<void> _confirmDeleteCourse(
    ...
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Course $code deleted')));
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
                              ...
                              Row(
                                ...
                                children: [
                                  Switch(
                                    value: available,
                                    onChanged: (val) =>
                                        _toggleAvailableNextSemester(doc, val),
                                  ),
                                  IconButton(
                                    ...
                                    onPressed: () =>
                                        _confirmDeleteCourse(doc, code, name),
                                  ),
                                ],
                              ),
                            ],
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          Navigator.of(context).push(
            MaterialPageRoute(builder: (_) => const SuperAdminAddCoursePage()),
          );
        },
